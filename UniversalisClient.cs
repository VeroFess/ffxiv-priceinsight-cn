...
public sealed class UniversalisClient : IDisposable {
    ...
    public async Task<MarketBoardData?> GetMarketBoardData(string scope, uint homeWorldId, ulong itemId) {
        try {
            using var result = await httpClient.GetAsync($"https://api.universalis.binklac.cn/v2/{scope}/{itemId}?fields={RequiredFields}");
    ...
    public async Task<Dictionary<uint, MarketBoardData>?> GetMarketBoardDataList(string scope, uint homeWorldId, IList<uint> itemId) {
        ...
        try {
            using var result = await httpClient.GetAsync($"https://api.universalis.binklac.cn/v2/{scope}/{string.Join(',', itemId.Select(i => i.ToString()))}?fields={RequiredFieldsMulti}");
